...
export async function activate(context: vscode.ExtensionContext) {
  outputChannel = vscode.window.createOutputChannel('Rox Space');
  ...
  const attemptMoveChatToRightSidebar = async (): Promise<'moved' | 'unsupported' | 'failed'> => {
    ...
    try {
      ...
      outputChannel?.appendLine(
        `[Rox Space] Failed moving chat view to right sidebar (command=${moveCommandId}): ${error instanceof Error ? error.message : String(error)}`
      );
      return 'failed';
    }
  };
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('openchamber.openSidebar', async () => {
      ...
      try {
        ...
        outputChannel?.appendLine(`[Rox Space] workbench.view.extension.openchamber failed: ${e}`);
      }

      try {
        await vscode.commands.executeCommand('openchamber.chatView.focus');
      } catch (e) {
        outputChannel?.appendLine(`[Rox Space] openchamber.chatView.focus failed: ${e}`);
        vscode.window.showErrorMessage(t('Rox Space: Failed to open sidebar - {0}', String(e)));
        return false;
      }

      if (!chatViewProvider?.hasResolvedView()) {
        outputChannel?.appendLine('[Rox Space] Chat sidebar focus completed before the webview was resolved');
        vscode.window.showWarningMessage(t('Rox Space: Chat sidebar is not ready'));
        return false;
      }
  ...
  const revealChatViewForPayload = async () => {
    ...
    if (!chatViewProvider?.hasResolvedView()) {
      outputChannel?.appendLine('[Rox Space] Chat sidebar webview was disposed before payload delivery');
      vscode.window.showWarningMessage(t('Rox Space: Chat sidebar is not ready'));
      return false;
    }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('openchamber.openActiveSessionInEditor', () => {
      if (!activeSessionId) {
        vscode.window.showInformationMessage(t('Rox Space: No active session'));
        return;
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('openchamber.restartApi', async () => {
      try {
        ...
        vscode.window.showInformationMessage(t('Rox Space: API connection restarted'));
      } catch (e) {
        vscode.window.showErrorMessage(t('Rox Space: Failed to restart API - {0}', String(e)));
      }
    })
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('openchamber.addToContext', async () => {
      ...
      if (!editor) {
        vscode.window.showWarningMessage(t('Rox Space [Add to Context]: No active editor'));
        return;
      }
      ...
      if (!selectedText) {
        vscode.window.showWarningMessage(t('Rox Space [Add to Context]: No text selected'));
        return;
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('openchamber.attachExplorerToChat', async (resource?: vscode.Uri, resources?: vscode.Uri[]) => {
      ...
      if (attachedFiles.length === 0) {
        vscode.window.showWarningMessage(t('Rox Space: No file selected to mention'));
        return;
      }
      ...
      if (skippedEntries.length > 0) {
        vscode.window.showInformationMessage(t('Rox Space: Some selected entries were skipped (folders or unsupported resources)'));
      }
    })
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('openchamber.explain', async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
        vscode.window.showWarningMessage(t('Rox Space [Explain]: No active editor'));
        return;
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('openchamber.improveCode', async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
        vscode.window.showWarningMessage(t('Rox Space [Improve Code]: No active editor'));
        return;
      }
      ...
      if (!selectedText) {
        vscode.window.showWarningMessage(t('Rox Space [Improve Code]: No text selected'));
        return;
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('openchamber.newSession', async (directory?: unknown) => {
      ...
      if (!folderPath && candidates.length === 0) {
        vscode.window.showInformationMessage('Rox Space: No folder is open. Open a folder to start a new session.');
        return;
      }
      ...
      if (openCodeManager) {
        ...
        if (!result.success) {
          vscode.window.showErrorMessage(`Rox Space: ${result.error}`);
          return;
        }
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('openchamber.showOpenCodeStatus', async () => {
      ...
      const lines = [
        ...
        `Rox Space version: ${extensionVersion || '(unknown)'}`,
