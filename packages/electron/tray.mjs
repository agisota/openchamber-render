...
//
// Surfaces a glanceable, always-visible view of Rox Space's live state:
...
const computeTooltip = (counts, sessionCount) => {
  if (sessionCount === 0) return 'Rox Space — no active sessions';
  const bits = [];
  ...
  return `Rox Space — ${sessionCount} session${sessionCount === 1 ? '' : 's'}${suffix}`;
};
...
export const createTrayController = ({ idleIconPath, unseenIconPath, breathIconPaths, statusIconPaths, onAction }) => {
  ...
  const buildMenu = (snapshot) => {
    ...
    template.push(
      ...
      { label: 'Show Rox Space', click: () => onAction({ type: 'show-main-window' }) },
      { type: 'separator' },
      { label: 'Quit Rox Space', click: () => onAction({ type: 'quit' }) },
    );
