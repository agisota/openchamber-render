...
2. Add a server and choose its type:
   - **local** — Rox Space runs a command on your machine. You give it the command to run and, if needed, environment variables.
   - **remote** — Rox Space connects to a URL someone else hosts. You give it the URL and any headers it needs (for example an auth token).
