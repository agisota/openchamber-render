---
title: Tunnels
description: Exposez Rox Space en sécurité pour l’accès distant et mobile.
---

# Tunnels

Un tunnel est un lien public vers votre Rox Space, pour y accéder depuis votre téléphone ou un autre réseau. Utilisez `openchamber tunnel` pour en créer un pour une instance en cours d’exécution.

## Prérequis

Rox Space démarre la CLI du fournisseur de tunnel sur votre machine. Installez d’abord le fournisseur que vous voulez utiliser :
...
Quand le tunnel est actif, `status` affiche une URL publique. Ouvrez-la, ou scannez le QR code, pour joindre Rox Space depuis n’importe où.

Par défaut, Rox Space affiche un QR code dans les sessions TTY interactives. Utilisez `--qr` pour forcer la sortie QR, ou `--no-qr` pour la désactiver.
...
- un tunnel actif par instance Rox Space (port)
...
- [PWA et mobile](/mobile/) — joindre Rox Space depuis votre téléphone
