...
description: Connectez Rox Space à un serveur OpenCode local ou distant.
---

# Serveur OpenCode

Rox Space fonctionne par-dessus un serveur OpenCode. Par défaut, il en démarre un pour vous, donc vous n’avez rien à faire. Vous n’avez besoin de cette page que si vous voulez pointer Rox Space vers un serveur que vous exécutez déjà, ou gérer celui qu’il démarre.

## Comment Rox Space trouve un serveur

Au démarrage, Rox Space cherche un serveur dans cet ordre :
...
Définissez ces variables avant de lancer Rox Space :
...
- `OPENCODE_SKIP_START=true` — indique à Rox Space de ne pas démarrer son propre serveur.

Si vous voulez seulement changer le port, définissez `OPENCODE_PORT` au lieu de `OPENCODE_HOST`.

Si `OPENCODE_HOST` n’a pas de port ou contient un chemin, Rox Space l’ignore et revient au démarrage de son propre serveur. Surveillez les logs de démarrage pour un avertissement `[config]` si une connexion attendue ne se fait pas.
...
## Lancer Rox Space à la connexion

Utilisez `startup enable` pour installer un service utilisateur natif. Rox Space utilise `launchd` sur macOS, `systemd --user` sur Linux et le Planificateur de tâches sur Windows.
...
Pour créer un lien de connexion pour une autre application Rox Space, utilisez :
...
Pendant que le serveur démarre ou redémarre, Rox Space affiche l’état « OpenCode redémarre » et met les requêtes en pause jusqu’à ce qu’il soit prêt. C’est normal juste après un lancement ou un redémarrage. Si l’état ne disparaît jamais, consultez [Connexion à OpenCode](/troubleshooting/opencode-connection/).
