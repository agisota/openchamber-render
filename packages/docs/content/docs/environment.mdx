...
description: Configure Rox Space and OpenCode integration with environment variables.
---

# Environment Variables

Rox Space reads these environment variables at startup. For startup services, `openchamber startup enable` snapshots the current environment by default, so rerun it after changing variables you want the service to use.

## Rox Space server

### `OPENCHAMBER_HOST`

Bind address for the Rox Space web server. Use `0.0.0.0` to allow access from other machines.
...
Starts Rox Space in headless mode when set to `true` or `1`. API routes stay available for desktop and mobile clients, but the browser UI is not served.

### `OPENCHAMBER_DATA_DIR`

Overrides the Rox Space data directory. The default is `~/.config/openchamber`.
...
Connects Rox Space to an existing OpenCode server. The value must be an `http` or `https` origin with an explicit port and no path, query, or hash. `OPENCODE_HOST` takes precedence over `OPENCODE_PORT`.
...
Prevents Rox Space from starting its own OpenCode server when set to `true`.

### `OPENCHAMBER_OPENCODE_HOSTNAME`

Bind hostname for the OpenCode server managed by Rox Space. The default is `127.0.0.1`.

### `OPENCODE_BINARY`

Path to the `opencode` executable Rox Space should run.
...
Rox Space-specific alias for selecting the WSL distro. `OPENCODE_WSL_DISTRO` takes precedence when both are set.
...
Shell executable used for Rox Space terminal sessions.

### `OPENCHAMBER_GIT_BINARY`

Git executable used by Rox Space Git features.

### `GIT_BINARY`

Alternative Git executable override. Prefer `OPENCHAMBER_GIT_BINARY` for Rox Space-specific configuration.
...
API key used by Rox Space voice features that call OpenAI-compatible services.
...
ngrok auth token used by Rox Space tunnel commands. You can also configure ngrok with `ngrok config add-authtoken <token>`.

## Runtime helpers

### `BUN_BINARY`

Bun executable Rox Space should use when spawning daemon processes.

### `BUN_INSTALL`

Bun installation root. Rox Space uses it to find `bin/bun` for daemon startup and updates.
