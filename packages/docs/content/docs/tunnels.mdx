---
title: Tunnels
description: Expose Rox Space safely for remote and mobile access.
---

# Tunnels

A tunnel is a public link to your Rox Space, so you can reach it from your phone or another network. Use `openchamber tunnel` to create one for a running instance.

## Prerequisites

Rox Space starts the tunnel provider CLI on your machine. Install the provider you want to use first:
...
When the tunnel is up, `status` shows a public URL. Open it, or scan the QR code, to reach Rox Space from anywhere.

By default, Rox Space prints a QR code in interactive TTY sessions. Use `--qr` to force QR output, or `--no-qr` to disable it.
...
- one active tunnel per Rox Space instance (port)
...
- [PWA & Mobile](/mobile/) — reach Rox Space from your phone
