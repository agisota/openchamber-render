...
description: Configura la integración de Rox Space y OpenCode con variables de entorno.
---

# Variables de entorno

Rox Space lee estas variables al arrancar. Para servicios de inicio, `openchamber startup enable` guarda por defecto una copia del entorno actual, así que vuelve a ejecutarlo después de cambiar variables que quieras usar en el servicio.

## Servidor Rox Space

### `OPENCHAMBER_HOST`

Dirección donde escucha el servidor web de Rox Space. Usa `0.0.0.0` para permitir acceso desde otras máquinas.
...
Inicia Rox Space en modo headless cuando vale `true` o `1`. Las rutas API siguen disponibles para clientes de escritorio y móviles, pero no se sirve la UI del navegador.

### `OPENCHAMBER_DATA_DIR`

Cambia el directorio de datos de Rox Space. Por defecto es `~/.config/openchamber`.
...
Conecta Rox Space a un servidor OpenCode existente. Debe ser un origin `http` o `https` con puerto explícito y sin ruta, query ni hash. `OPENCODE_HOST` tiene prioridad sobre `OPENCODE_PORT`.
...
Evita que Rox Space arranque su propio servidor OpenCode cuando vale `true`.

### `OPENCHAMBER_OPENCODE_HOSTNAME`

Hostname para el servidor OpenCode gestionado por Rox Space. Por defecto es `127.0.0.1`.

### `OPENCODE_BINARY`

Ruta al ejecutable `opencode` que debe usar Rox Space.
...
Alias específico de Rox Space para elegir la distro WSL. `OPENCODE_WSL_DISTRO` tiene prioridad si ambas están definidas.
...
Ejecutable de shell para las sesiones de terminal en Rox Space.

### `OPENCHAMBER_GIT_BINARY`

Ejecutable de Git usado por las funciones Git de Rox Space.

### `GIT_BINARY`

Override alternativo para el ejecutable de Git. Para ajustes específicos de Rox Space, prefiere `OPENCHAMBER_GIT_BINARY`.
...
API key para funciones de voz de Rox Space que llaman a servicios compatibles con OpenAI.
...
Token de autenticación de ngrok usado por los comandos de túnel de Rox Space. También puedes configurarlo con `ngrok config add-authtoken <token>`.

## Ayudas de runtime

### `BUN_BINARY`

Ejecutable de Bun que Rox Space debe usar al iniciar procesos daemon.

### `BUN_INSTALL`

Raíz de instalación de Bun. Rox Space la usa para encontrar `bin/bun` al iniciar daemons y actualizar.
