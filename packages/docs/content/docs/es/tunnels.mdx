---
title: Túneles
description: Expón Rox Space de forma segura para acceso remoto y móvil.
---

# Túneles

Un túnel es un enlace público a tu Rox Space, para que puedas acceder a él desde tu teléfono u otra red. Usa `openchamber tunnel` para crear uno para una instancia en marcha.

## Requisitos previos

Rox Space inicia el CLI del proveedor de túnel en tu máquina. Instala primero el proveedor que quieras usar:
...
Cuando el túnel está activo, `status` muestra una URL pública. Ábrela, o escanea el código QR, para acceder a Rox Space desde cualquier lugar.

Por defecto, Rox Space imprime un código QR en sesiones TTY interactivas. Usa `--qr` para forzar la salida del QR, o `--no-qr` para desactivarla.
...
- un único túnel activo por instancia de Rox Space (puerto)
...
- [PWA y acceso móvil](/es/mobile/) — accede a Rox Space desde tu teléfono
