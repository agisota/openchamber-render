...
description: Conecta Rox Space a un servidor de OpenCode local o remoto.
---

# OpenCode Server

Rox Space se ejecuta sobre un servidor de OpenCode. De forma predeterminada inicia uno por ti, así que no tienes que hacer nada. Solo necesitas esta página si quieres apuntar Rox Space a un servidor que ya ejecutas, o gestionar el que él inicia.

## Cómo encuentra Rox Space un servidor

Cuando Rox Space arranca, busca un servidor en este orden:
...
Define esto antes de iniciar Rox Space:
...
- `OPENCODE_SKIP_START=true` — le indica a Rox Space que no inicie su propio servidor.

Si solo necesitas cambiar el puerto, define `OPENCODE_PORT` en lugar de `OPENCODE_HOST`.

Si a `OPENCODE_HOST` le falta el puerto o tiene una ruta, Rox Space lo ignora y recurre a iniciar su propio servidor. Observa los registros de arranque por si aparece una advertencia `[config]` cuando una conexión que esperabas no ocurrió.
...
## Iniciar Rox Space al iniciar sesión

Usa `startup enable` para instalar un servicio nativo de usuario. Rox Space usa `launchd` en macOS, `systemd --user` en Linux y Task Scheduler en Windows.
...
Para crear un enlace de conexión para otra app de Rox Space, usa:
...
Mientras el servidor se inicia o reinicia, Rox Space muestra un estado "OpenCode is restarting" y pausa las solicitudes hasta que esté listo. Esto es normal justo después de iniciar o reiniciar. Si nunca desaparece, consulta [Conexión de OpenCode](/es/troubleshooting/opencode-connection/).
