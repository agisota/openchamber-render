---
title: Proxy inverso
description: Configura Rox Space correctamente detrás de Nginx, Nginx Proxy Manager u otro proxy inverso.
---

# Proxy inverso

Usa esta página si ejecutas Rox Space detrás de Nginx, Nginx Proxy Manager, Caddy, Cloudflare u otro proxy inverso.

## Antes de ponerle un proxy

1. Confirma primero que Rox Space funciona directamente.
...
- Desactiva gzip en el proxy si Rox Space ya comprime las respuestas.
...
- Rox Space accesible directamente en la LAN
...
- Rox Space comprime las respuestas HTTP con gzip (umbral de 1 KB).
...
Las rutas de streaming SSE están excluidas de la compresión por Rox Space, pero la CDN aún puede almacenarlas en búfer. Consulta la documentación de tu CDN para saber cómo desactivar el búfer en las rutas SSE.
