...
Rox Space can read hosts from your local SSH config, the same place used by commands like `ssh work-server`.
...
Rox Space creates a remote instance using that SSH command. When it reaches **ready**, the remote Rox Space UI opens in the desktop app.
...
Rox Space supports three forward types:
...
Choose **Dynamic (-D)** when you want other tools on your computer to browse through the remote machine. Rox Space opens a local SOCKS proxy port for that SSH connection.
...
- [Remote Instances](/remote-instances/) — connect the desktop app to Rox Space on another machine
