---
title: Tunele
description: Bezpiecznie udostępnij Rox Space do dostępu zdalnego i mobilnego.
---

# Tunele

Tunel to publiczny link do Twojego Rox Space, dzięki któremu możesz dotrzeć do niego z telefonu lub z innej sieci. Użyj `openchamber tunnel`, aby utworzyć go dla działającej instancji.

## Wymagania wstępne

Rox Space uruchamia CLI dostawcy tunelu na Twoim komputerze. Najpierw zainstaluj dostawcę, którego chcesz użyć:
...
Gdy tunel działa, `status` pokazuje publiczny adres URL. Otwórz go lub zeskanuj kod QR, aby dotrzeć do Rox Space z dowolnego miejsca.

Domyślnie Rox Space wypisuje kod QR w interaktywnych sesjach TTY. Użyj `--qr`, aby wymusić wyświetlenie kodu QR, lub `--no-qr`, aby je wyłączyć.
...
- jeden aktywny tunel na instancję Rox Space (port)
...
- [PWA i dostęp z telefonu](/pl/mobile/) — korzystaj z Rox Space z telefonu
