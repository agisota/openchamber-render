---
title: Reverse proxy
description: Skonfiguruj Rox Space poprawnie za Nginx, Nginx Proxy Manager lub innym reverse proxy.
---

# Reverse proxy

Skorzystaj z tej strony, jeśli uruchamiasz Rox Space za Nginx, Nginx Proxy Manager, Caddy, Cloudflare lub innym reverse proxy.

## Zanim ustawisz proxy

1. Najpierw potwierdź, że Rox Space działa bezpośrednio.
...
- Wyłącz gzip na proxy, jeśli Rox Space już kompresuje odpowiedzi.
...
- Rox Space dostępny bezpośrednio w LAN
...
- Rox Space kompresuje odpowiedzi HTTP za pomocą gzip (próg 1 KB).
...
Trasy strumieniowania SSE są wyłączone z kompresji przez Rox Space, ale CDN może je nadal buforować. Sprawdź dokumentację swojego CDN, jak wyłączyć buforowanie na ścieżkach SSE.
