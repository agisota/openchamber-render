...
description: Napraw brak połączenia Rox Space z serwerem OpenCode.
---

# Połączenie z OpenCode

Jeśli Rox Space się ładuje, ale nigdy nie przechodzi poza „OpenCode uruchamia się ponownie”, albo czat nie odpowiada, serwer, z którym się komunikuje, jest nieosiągalny. Przejdź przez te kroki.
...
Jeśli ustawiłeś Rox Space tak, aby używał istniejącego serwera, sprawdź ponownie konfigurację z [OpenCode Server](/pl/opencode-server/):

- `OPENCODE_HOST` musi zawierać port, bez ścieżki — na przykład `http://localhost:4096`
- ustaw `OPENCODE_SKIP_START=true`, aby Rox Space nie uruchamiał też własnego
- jeśli adres jest nieprawidłowy, Rox Space go ignoruje i uruchamia własny serwer — poszukaj ostrzeżenia `[config]` w logach
...
- [OpenCode Server](/pl/opencode-server/) — jak Rox Space znajduje serwer i nim zarządza
