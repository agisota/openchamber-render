---
title: Túneis
description: Exponha o Rox Space com segurança para acesso remoto e móvel.
---

# Túneis

Um túnel é um link público para o seu Rox Space, para que você possa acessá-lo pelo celular ou de outra rede. Use `openchamber tunnel` para criar um para uma instância em execução.

## Pré-requisitos

O Rox Space inicia o CLI do provedor de túnel na sua máquina. Instale primeiro o provedor que você quer usar:
...
Quando o túnel estiver no ar, o `status` mostra uma URL pública. Abra-a, ou escaneie o QR code, para acessar o Rox Space de qualquer lugar.

Por padrão, o Rox Space imprime um QR code em sessões TTY interativas. Use `--qr` para forçar a saída do QR, ou `--no-qr` para desativá-la.
...
- um único túnel ativo por instância do Rox Space (porta)
...
- [PWA e Acesso Móvel](/pt-br/mobile/) — acesse o Rox Space pelo celular
