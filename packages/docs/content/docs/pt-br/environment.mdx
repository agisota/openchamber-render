...
description: Configure a integração do Rox Space e do OpenCode com variáveis de ambiente.
---

# Variáveis de ambiente

O Rox Space lê estas variáveis na inicialização. Para serviços de inicialização, `openchamber startup enable` salva por padrão um snapshot do ambiente atual, então execute de novo após mudar variáveis que o serviço deve usar.

## Servidor Rox Space

### `OPENCHAMBER_HOST`

Endereço onde o servidor web do Rox Space escuta. Use `0.0.0.0` para permitir acesso de outras máquinas.
...
Inicia o Rox Space em modo headless quando definido como `true` ou `1`. As rotas de API continuam disponíveis para clientes desktop e mobile, mas a UI do navegador não é servida.

### `OPENCHAMBER_DATA_DIR`

Altera o diretório de dados do Rox Space. O padrão é `~/.config/openchamber`.
...
Conecta o Rox Space a um servidor OpenCode existente. O valor deve ser um origin `http` ou `https` com porta explícita e sem path, query ou hash. `OPENCODE_HOST` tem prioridade sobre `OPENCODE_PORT`.
...
Impede o Rox Space de iniciar seu próprio servidor OpenCode quando definido como `true`.

### `OPENCHAMBER_OPENCODE_HOSTNAME`

Hostname do servidor OpenCode gerenciado pelo Rox Space. O padrão é `127.0.0.1`.

### `OPENCODE_BINARY`

Caminho para o executável `opencode` que o Rox Space deve usar.
...
Alias específico do Rox Space para escolher a distro WSL. `OPENCODE_WSL_DISTRO` tem prioridade quando ambas estão definidas.
...
Executável de shell usado nas sessões de terminal do Rox Space.

### `OPENCHAMBER_GIT_BINARY`

Executável do Git usado pelos recursos Git do Rox Space.

### `GIT_BINARY`

Override alternativo para o executável do Git. Para configuração específica do Rox Space, prefira `OPENCHAMBER_GIT_BINARY`.
...
API key usada por recursos de voz do Rox Space que chamam serviços compatíveis com OpenAI.
...
Token de autenticação do ngrok usado pelos comandos de túnel do Rox Space. Você também pode configurar com `ngrok config add-authtoken <token>`.

## Auxiliares de runtime

### `BUN_BINARY`

Executável do Bun que o Rox Space deve usar ao iniciar processos daemon.

### `BUN_INSTALL`

Raiz da instalação do Bun. O Rox Space usa isso para encontrar `bin/bun` ao iniciar daemons e atualizar.
