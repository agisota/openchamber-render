...
Rendering and hosting happen in the configured Rox Space docs host (`apps/docs`).
...
- creates `rox-space-docs-source-<sha>.tar.gz`
...
to the configured Rox Space docs host so it re-syncs and redeploys the docs. This fires on
...
- `ROX_SPACE_DOCS_REPO_TOKEN` — a token with `contents: write` (classic
  `repo` scope, or fine-grained with Contents: read & write) on
  the configured Rox Space docs host repository. **Without it the dispatch step is skipped**
...
- `source_repo`
- `source_ref` — the ref the website checks out from this repository (`main` on a
  push, the tag on a release)
- `archive_name`

The docs host `deploy-docs.yml` listens for this event
(`repository_dispatch: types: [docs_source_updated]`), checks out
this repository at `source_ref`, runs `docs:sync`, builds `apps/docs`, and deploys
to Cloudflare Pages. That repo needs its own secrets: `ROX_SPACE_REPO_TOKEN`
