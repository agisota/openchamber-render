...
This package is docs content source-of-truth for Rox Space.
...
- Lead with the task, not background. The first line should say what the page is
  for ("Use `openchamber tunnel` to expose a running Rox Space instance.").
...
- A basic page should fit in a screen or two. Long, dense reference pages (like
  ...
  this page if you run Rox Space behind...").
...
You should land on the Rox Space session list. If you see it, the server is
running.
```
...
- Explain a term the first time it appears, in parentheses, in everyday words:
  - good: start a tunnel (a public link to your local Rox Space)
...
2. Add frontmatter at top:
   ...
   ---
   title: Remote Access
   description: Access Rox Space from outside your local network.
   ---
   ```
...
The docs are translated into the same languages the Rox Space app ships in.
...
This locale set is mirrored in the configured Rox Space docs host at
`apps/docs/astro.config.mjs` (`locales`). If a language is added or removed,
update both places.
...
- brand and product nouns: Rox Space, OpenCode, VS Code, PWA, GitHub,
  macOS, SSH
...
## Sync into Rox Space docs host

The configured Rox Space docs host renders/deploys docs via Starlight in `apps/docs`.
