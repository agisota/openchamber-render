...
process.env.OPENCHAMBER_UPDATE_API_URL = 'https://updates.rox.space/v1/update/check';
process.env.OPENCHAMBER_CHANGELOG_URL = 'https://updates.rox.space/CHANGELOG.md';
...
describe('checkForUpdates', () => {
  ...
  it('returns available=true when both API and npm confirm a newer version', async () => {
    fetchMock
      .when('updates.rox.space/v1/update/check', {
        ok: true,
        json: async () => ({
      ...
      .when('updates.rox.space/CHANGELOG.md', {
        ok: true,
  ...
  it('returns available=false when npm only has a prerelease of the current version', async () => {
    fetchMock
      .when('updates.rox.space/v1/update/check', Promise.reject(new Error('Network error')))
      .when('registry.npmjs.org', {
        ok: true,
        json: async () => ({
  ...
  it('returns available=false when API says no update and versions match', async () => {
    fetchMock.when('updates.rox.space/v1/update/check', {
      ok: true,
      json: async () => ({
