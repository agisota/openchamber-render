...
export const ProjectActionsButton = ({
  ...
  const [actions, setActions] = React.useState<RoxSpaceProjectAction[]>([]);
  ...
  const autoDiscoverAction = React.useMemo<RoxSpaceProjectAction>(() => ({
  ...
  const getOrCreateActionTab = React.useCallback(async (action: RoxSpaceProjectAction, options: { revealTerminal?: boolean } = {}) => {
  ...
  const runAction = React.useCallback(async (action: RoxSpaceProjectAction) => {
    ...
    try {
      const discovered = action.id === AUTO_DISCOVER_ACTION_ID
        ? await (async (): Promise<RoxSpaceProjectAction> => {
  ...
  const stopAction = React.useCallback(async (action: RoxSpaceProjectAction) => {
    const runKey = toProjectActionRunKey(normalizedDirectory, action.id);
  ...
  const handleSelectAction = React.useCallback((action: RoxSpaceProjectAction, toggleStopIfRunning = false) => {
