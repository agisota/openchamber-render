...
import { parseProjectPlanMarkdown } from '@/lib/roxSpaceConfig';
...
export const PlanView: React.FC<PlanViewProps> = ({ targetPath = null }) => {
  ...
  const handleConfirmPlanSend = React.useCallback(
    async (execution: TodoSendExecution) => {
      ...
      try {
        ...
        if (pendingPlanSend.target === 'worktree') {
          ...
          initializeNewRoxSpaceSession(sessionResult.id, useConfigStore.getState().agents ?? []);
        }
    ...
    [canCreateWorktree, createSession, currentProjectRef, initializeNewRoxSpaceSession, pendingPlanSend, resolvedPath, routeToChat, sendMessage, sendPromptTitle, setCurrentSession]
  );
