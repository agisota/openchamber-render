...
Use this guide when running Rox Space behind Nginx, Nginx Proxy Manager, Caddy, Cloudflare, or another reverse proxy.

## Before you proxy it

1. Confirm Rox Space works directly first.
...
- Disable gzip on the proxy if Rox Space is already compressing responses.
...
- Rox Space reachable directly on LAN
...
- Rox Space compresses HTTP responses with gzip (threshold 1 KB).
...
SSE streaming routes are excluded from compression by Rox Space, but the CDN may still buffer them. Check your CDN documentation for how to disable buffering on SSE paths.
